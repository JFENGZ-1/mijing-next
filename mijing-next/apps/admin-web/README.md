...
```powershell
pnpm install
pnpm --filter @mijing/admin-web contract:accept
pnpm --filter @mijing/admin-web dev
```
...
```powershell
pnpm --filter @mijing/admin-web build
```
...
- 独立 worktree 联调时可通过 `MIJING_OPENAPI_PATH` 指向接口任务正在修改的 `openapi.yaml`，无需复制或改写上游文件。
...
```powershell
$env:MIJING_OPENAPI_PATH = 'D:\path\to\active-worktree\mijing-next\docs\openapi.yaml'
pnpm --filter @mijing/admin-web contract:sync
```
