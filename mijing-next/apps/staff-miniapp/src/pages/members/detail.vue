...
function openRechargeAmount() {
  ...
  uni.navigateTo({
    url: `/pages/members/recharge-amount?id=${memberId.value}&name=${name}&avatar=${avatar}&total=${total}`,
  });
}
function openCourseDetail(mode: 0 | 1 | 2) {
  ...
  uni.navigateTo({
    url: `/pages/members/course-stats?id=${memberId.value}&mode=${mode}&name=${name}&avatar=${avatar}&month=${encodeURIComponent(month)}&total=${encodeURIComponent(total)}`,
  });
}
function openPoints() {
  if (!memberId.value) return;
  ...
  const name = encodeURIComponent(nameText.value);
  const avatar = encodeURIComponent(member.value?.avatarUrl || "");
  uni.navigateTo({ url: `/pages/members/points?id=${memberId.value}&name=${name}&avatar=${avatar}` });
}
...
function openCardDetail(card: StaffMemberCardSummary) {
  if (!memberId.value) return;
  uni.navigateTo({ url: `/pages/members/card-detail?memberId=${memberId.value}&memberCardId=${card.id}` });
}
...
function openArchivedCards() {
  ...
  uni.navigateTo({ url: `/pages/members/archived-cards/index?memberId=${memberId.value}&title=${title}` });
}
