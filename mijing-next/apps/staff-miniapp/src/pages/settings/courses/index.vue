...
function openCreate() {
  ...
  if (isPrivateTab.value) {
    uni.navigateTo({ url: "/pages/settings/courses/private-edit" });
    return;
  }
  uni.navigateTo({ url: "/pages/settings/courses/edit?type=group" });
}
...
function openProfileEdit(profile: CoachPrivateProfile) {
  if (!canWrite.value) return;
  uni.navigateTo({ url: `/pages/settings/courses/private-edit?id=${profile.id}` });
}
