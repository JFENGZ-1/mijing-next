...
function openDetail(item: ReportCoachMonthlyRank["items"][number]) {
  uni.navigateTo({
    url: `/subpackages/report/coaches/detail?staffId=${item.staffId}&staffName=${encodeURIComponent(staffLabel(item.staffName, item.staffId))}&year=${selectedYear.value}&month=${selectedMonth.value}`,
  });
}
