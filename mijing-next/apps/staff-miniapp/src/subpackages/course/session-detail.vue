...
onShareAppMessage(() => ({
  ...
  path: `/subpackages/course/session-detail?id=${sessionId.value}`,
}));
...
function openMember(memberId: number) {
  ...
  uni.navigateTo({ url: `/subpackages/members/detail?id=${memberId}` });
}
...
function openEdit() {
  uni.navigateTo({ url: `/subpackages/course/session-form?id=${sessionId.value}` });
}
</script>
