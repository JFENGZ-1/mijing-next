...
function openCreate() {
  uni.navigateTo({ url: "/subpackages/settings/staff/edit" });
}

function openEdit(item: StaffDirectoryListItem) {
  uni.navigateTo({ url: `/subpackages/settings/staff/edit?id=${item.id}` });
}

function openInvite(item: StaffDirectoryListItem) {
  // 去邀请：进入编辑页内的「邀请绑定微信」分享
  uni.navigateTo({ url: `/subpackages/settings/staff/edit?id=${item.id}&invite=1` });
}
