...
ensure_base_tools() {
  ...
  for command_name in git curl tar xz unzip awk sort sha256sum; do
...
if [ -z "$NODE_VERSION" ] || ! version_at_least "$NODE_VERSION" "20.19.0"; then
  log "安装 Node.js 22 官方 Linux 二进制包（仅用于构建 Web 后台）"
  install_node22
  NODE_VERSION="$(node -p 'process.versions.node' 2>/dev/null || true)"
fi
...
pnpm --filter @mijing/admin-web install --frozen-lockfile
...
[ -f "${ADMIN_DIST}/index.html" ] || fail "Web 后台构建产物不存在。"
...
printf '\n\033[1;32mLaravel 服务端、Web 后台与后台任务部署完成。\033[0m\n'
printf '宝塔站点运行目录：%s\n' "$PUBLIC_DIR"
