...
const injectFn = `function(){wx.setStorageSync('__shot_purchase',${JSON.stringify({
  name: payload.name,
...
pause(4);
console.log(run('automation_evaluate', `--project "${memberProject}" --fn-source ${JSON.stringify(injectFn)}`));
console.log(
  run(
    'automation_evaluate',
...
console.log('member seeded-success retake done');
