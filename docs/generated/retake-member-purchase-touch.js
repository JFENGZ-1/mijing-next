...
function touch(project, x, y) {
  run('automation_element_action', `--project "${project}" --selector page --action touchstart --x ${x} --y ${y}`);
  run('automation_element_action', `--project "${project}" --selector page --action touchend --x ${x} --y ${y}`);
}
...
for (const [x, y] of points) {
  try {
    ...
    console.log('touch fail', x, y, e.message);
  }
}
