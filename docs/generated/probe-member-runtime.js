...
pause(3);
console.log(run('automation_runtime_info', `--project "${memberProject}" --action systemInfo`));
run('automation_element_action', `--project "${memberProject}" --selector .product-actions button --action tap`);
pause(1);
console.log(run('automation_runtime_info', `--project "${memberProject}" --action currentPage`));
