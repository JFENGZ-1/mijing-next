...
class MemberCardStateService
{
    ...
    public function freeze(Staff $staff, Site $site, MemberCard $memberCard, array $payload): array
    {
        ...
        return $this->mutate($staff, $site, $memberCard, $payload['commandKey'], function (MemberCard $locked) use ($staff, $site, $payload) {
    ...
    public function startHoliday(Staff $staff, Site $site, MemberCard $memberCard, array $payload): array
    {
        ...
        return $this->mutate($staff, $site, $memberCard, $payload['commandKey'], function (MemberCard $locked) use ($staff, $site, $payload) {
            ...
            $entry = EntitlementLedgerEntry::create([
                ...
                'command_key' => $payload['commandKey'],
                'reason' => $payload['reason'],
                'metadata' => ['plannedEndAt' => $plannedEndAt],
                'actor_staff_id' => $staff->id,
                'occurred_at' => now(),
            ]);
     ...
     * @return array{memberCard: MemberCard, ledgerEntryIds: list<int>, created: bool}
     */
    private function mutate(Staff $staff, Site $site, MemberCard $memberCard, string $commandKey, callable $mutator): array
    {
        return DB::transaction(function () use ($staff, $site, $memberCard, $commandKey, $mutator) {
            ...
            if ($existingEntry) {
                ...
                if ($ledgerEntryIds === [] && $existingEntry->metadata['validityEntryId'] ?? null) {
                    $ledgerEntryIds = [
                        (int) $existingEntry->metadata['validityEntryId'],
                        $existingEntry->id,
                    ];
                }
