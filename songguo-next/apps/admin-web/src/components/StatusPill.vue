...
const tone = computed(() => {
  ...
  if (/pending|closing|waitlisted|待|候补|到期|预约|未开始/.test(value)) return "warning";
  if (/disabled|cancelled|absent|closed|frozen|voided|departed|expired|exhausted|停|冻结|失败|作废|离职/.test(value)) return "danger";
  return "neutral";
});
</script>
