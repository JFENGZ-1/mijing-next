...
**Sources:** `songguo-next/apps/server/routes/api.php`, `songguo-next/docs/openapi.yaml` (287 `operationId`s; `OpenApiRouteContractTest` asserts >200 ops, ≥95% route coverage), `docs/traceability-apis.csv` (10 `REJECT` rows excluded), frontend grep across `staff-miniapp/src` and `member-miniapp/src` (paths use `/staff/...` and `/member/...` via `@songguo/api-client`, not literal `/api/v1/` strings).
